...
// Mirrors pdf.PDFBridgeChunkBytes (wasm/redact/main.go, wasm/redact/session.go
// OutputRead): the Go encoder rejects any outputRead call whose maxBytes
// doesn't match this exactly.
const OUTPUT_CHUNK_BYTES = 1 << 20;

function responseMeta(response) {
  if (typeof response?.json !== "string") throw new Error("Redaction encoder returned an invalid response");
  try {
    return JSON.parse(response.json);
  } catch {
    throw new Error("Redaction encoder returned an invalid response");
  }
}
...
export async function streamRedactedPDF({
  ...
  try {
    ...
    const finishResponse = await call({ command: "finish" });
    sessionOpen = false;
    const { outputRevision, size } = responseMeta(finishResponse);
    if (!Number.isSafeInteger(outputRevision) || !Number.isSafeInteger(size) || size < 0) {
      throw new Error("Redaction encoder returned an invalid finish response");
    }
    ...
    try {
      for (;;) {
        const chunkResponse = await call({ command: "outputRead", outputRevision, maxBytes: OUTPUT_CHUNK_BYTES });
        const meta = responseMeta(chunkResponse);
        if (meta.done) break;
        const chunk = chunkResponse.data;
        if (!(chunk instanceof Uint8Array) || offset + chunk.byteLength > size) {
          throw new Error("Redaction encoder returned no PDF output");
        }
      ...
      try {
        await invoke({ command: "outputRelease", outputRevision });
      } catch {}
    }
    if (offset !== size) throw new Error("Redaction output size mismatch");
    return { data };
  } catch (error) {
