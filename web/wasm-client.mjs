...
export function createWasmClient(operation, { worker = null } = {}) {
  ...
  // Aborts whatever operation is currently in flight (and drains any pending
  // worker replies) WITHOUT touching workerInstance. Used by run() to
  // supersede a previous call while keeping the reusable worker alive.
  function abortActive(error = abortError()) {
