...
test("sequential run() calls reuse the same worker without terminating it", async (t) => {
  const previousWorker = globalThis.Worker;
  t.after(() => { globalThis.Worker = previousWorker; });
  let created = 0;
  let terminated = 0;
  ...
  await client.run(new Uint8Array([2]));
  await client.run(new Uint8Array([3]));

  assert.equal(created, 1);
  assert.equal(terminated, 0);
});

test("a superseding run() rejects the earlier in-flight operation without killing the worker", async (t) => {
  const previousWorker = globalThis.Worker;
  t.after(() => { globalThis.Worker = previousWorker; });
  let created = 0;
  let terminated = 0;
  let instance;
  class WorkerStub {
    constructor() { created++; instance = this; }
    postMessage(message) {
      // Never resolves on its own; only settled by explicit "done" injection
      // or by the client aborting it. Simulates an in-flight operation.
      this._pending = message;
    }
    resolvePending() {
      this.onmessage({ data: { type: "done", id: this._pending.id, result: {} } });
    }
    terminate() { terminated++; }
  }
  globalThis.Worker = WorkerStub;
  ...
  const first = client.run(new Uint8Array([1]));
  const second = client.run(new Uint8Array([2]));

  await assert.rejects(first, (error) => error?.name === "AbortError");

  instance.resolvePending();
  await second;
