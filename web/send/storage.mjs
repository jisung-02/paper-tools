...
export class FileSystemReceiveSink {
  ...
  async finish(file) {
    ...
    // Directory-kind outputs are saved directly to the user's chosen folder and shown
    // as "Saved" as soon as finish() succeeds, so a later abort() must not delete them.
    // OPFS-kind outputs stay owned until an explicit release() (see release() above),
    // since their content is only handed off as an in-memory Blob/object URL.
    if (this.kind !== "opfs") this.names.delete(state.outputName);
    return state.handle;
  }
