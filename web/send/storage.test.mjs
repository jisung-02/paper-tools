...
test("failed directory transfer removes all owned outputs and preserves unrelated files", async () => {
  ...
  assert.deepEqual([...directory.entries.keys()], ["keep.txt"]);
  assert.deepEqual(directory.removed.sort(), ["keep (2).txt", "partial.txt"]);
